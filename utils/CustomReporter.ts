...
export type BuildSummary = { runId: string; tests: Record<string, string> };
...
export type HealReport = {
    summary: string;
    repairedLocators?: Array<{ before: string; after: string }>;
};
...
const analyzeFailure = rcaModule?.analyzeFailure ?? (async (input: { title: string; file: string; error: string; stack?: string }) => ({
    summary: `AI RCA is unavailable for: ${input.title}`,
    rootCause: input.error,
    severity: 'LOW',
    priority: 'P2',
    fixes: ['Install the AI RCA modules to enable deeper failure analysis.'],
    confidence: 0,
    suggestedFix: 'Install the AI RCA modules to enable deeper failure analysis.',
} as RcaVerdict));

const analyzeFlaky = flakyModule?.analyzeFlaky ?? (async (_prev: BuildSummary, _curr: BuildSummary, _hasKey: boolean) => ({
    counts: { flaky: 0, failing: 0, total: 0 },
    flaky: [],
    summary: 'No flaky analysis available because the optional AI modules are not installed.',
    items: [],
} as FlakyResult));

const hasApiKey = providersModule?.hasApiKey ?? (() => false);
...
<body>
    ...
    private getStyles(): string {
        ...
        .main-tab-panel { display: none; overflow-x: auto; }
